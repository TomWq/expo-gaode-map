...
- **内置搜索** - 已集成到 `expo-gaode-map` 和 `expo-gaode-map-navigation`
...
搜索功能已内置在核心包和导航包中，不需要单独安装 `expo-gaode-map-search`。

::: warning 独立 Search 模块维护说明
`2.2.33` 是最后一个支持 `expo-gaode-map-search` 单独集成的版本。从下个版本开始，独立 search 包不再单独维护。
...
但运行时隐私合规仍需你在用户首次同意后手动设置一次；之后原生会自动恢复已同意状态。
:::
...
则不需要再调用 `initSDK({ androidKey, iosKey })`。首次安装仍必须先完成隐私合规；只有在使用 Web API 服务（或运行时手动设置 `webKey`）时，再调用 `initSDK({ webKey })`。
:::

::: warning 未配置原生 Key 时才需要运行时传入
如果既没有使用 Config Plugin，也没有手动在原生项目中配置 API Key，才需要在业务入口调用：
...
否则地图/定位/搜索/导航能力不可用。生产环境更推荐将移动端 Key 配置到原生项目中，避免暴露在 JavaScript 代码里。
:::
...
useEffect(() => {
  ...
  // 2. 已配置原生 Key 时，只有使用 Web API 服务才需要调用
...
安装 `expo-gaode-map` 或 `expo-gaode-map-navigation` 后：

```tsx
import { searchPOI, searchNearby } from 'expo-gaode-map';
// 如果使用导航包：
// import { searchPOI, searchNearby } from 'expo-gaode-map-navigation';
...
### 原生项目中配置 API Key 后还需要在代码中配置吗？

**不需要。**只要 API Key 已经通过 Config Plugin 或手动方式配置到原生项目中，就不需要再在 JavaScript 中传入 `androidKey` / `iosKey`。只有在使用 Web API 服务（`expo-gaode-map-web-api`）时，才需要传入 `webKey`：
...
### 为什么建议把移动端 Key 放在原生项目中？

将移动端 Key 配置在原生项目中**更安全**，避免将敏感信息暴露在 JavaScript 代码中。Expo 项目推荐使用 Config Plugin 自动写入；裸原生项目也可以手动写入 `AndroidManifest.xml` / `Info.plist`。Web API Key 除外，因为它需要在 JavaScript 中使用。
