...
如果使用了 Config Plugin，原生 API Key 会自动配置到原生项目中，SDK 也会默认自动初始化。  
首次安装仍必须先完成隐私合规。只有在使用 Web API 服务（或运行时手动设置 `webKey`）时，再调用 `initSDK({ webKey })`。
:::

::: warning 不使用 Config Plugin 的必做项
如果你没有使用 Config Plugin，必须在业务入口调用：
...
**不需要。**Config Plugin 会自动将 API Key 配置到原生项目中，且 SDK 默认自动初始化。只有在使用 Web API 服务（`expo-gaode-map-web-api`）时，才需要传入 `webKey`：
