...
| expo-gaode-map 版本 | Android 地图 SDK | iOS 地图 SDK | 离线地图说明 |
|---|---:|---:|---|
| `2.2.45` | `10.1.700` | `10.1.600` | 不涉及 `11.1.200` 起新增的离线地图商务开通要求 |
| `2.3.0+` | `11.2.000` | `11.2.000` | 离线地图下载需要联系高德商务单独开通权限 |

根据高德官方更新日志，地图 SDK `11.1.000` 暂不支持离线地图，`11.1.200` 恢复了离线地图下载功能。根据高德官方客服回复，从 `11.1.200` 开始，使用离线地图下载还需要单独开通商务权限。

- 不使用离线地图：可以使用 `expo-gaode-map@2.3.0+`，无需申请离线地图权限。
- 使用离线地图且已经开通权限：使用 `expo-gaode-map@2.3.0+`。
- 使用离线地图但不准备申请权限：固定使用 `expo-gaode-map@2.2.45`。

相关更新日志：[Android 3D 地图 SDK](https://lbs.amap.com/api/android-sdk/changelog/3d)、[iOS 3D 地图 SDK](https://lbs.amap.com/api/ios-sdk/changelog/3d)。权限政策可能调整，请以高德最新官方说明及商务答复为准。
