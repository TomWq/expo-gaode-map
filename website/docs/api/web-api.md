...
- **依赖**：可独立使用（不强依赖 `expo-gaode-map` / `expo-gaode-map-navigation`）
...
- ✅ **协同工作**：可与地图/导航模块协作并复用 `initSDK` 的 `webKey`
...
- `expo-gaode-map-navigation`（导航一体化包）
...
方式 A（推荐）：在 Web API 运行时显式传入 `config.key`。  
方式 B（可选）：在 `expo-gaode-map` / `expo-gaode-map-navigation` 的 `initSDK` 中下发 `webKey` 供 Web API 自动复用。
...
`createWebRuntime` / `GaodeWebAPI` 在创建实例时会按以下顺序解析 Web API Key：
...
兼容主类（推荐新项目优先使用 `createWebRuntime` / provider 工厂）。
...
A: 不需要。`expo-gaode-map-web-api` 可以独立使用，只要显式传入 `key` 即可。安装基础模块仅用于复用 `initSDK` 已配置的 `webKey`。
...
2. **初始化**：推荐显式传入 `key`；若已在基础模块 `initSDK({ webKey })` 配置，也可自动复用
