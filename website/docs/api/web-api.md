...
### Q: Web API 和内置原生搜索有什么区别？

A:
- **Web API**：纯 JavaScript，跨平台一致，但需要网络请求
- **内置原生搜索**：调用原生 SDK，性能更好，随 `expo-gaode-map` / `expo-gaode-map-navigation` 一起维护

建议优先使用 `expo-gaode-map` 或 `expo-gaode-map-navigation` 的内置原生搜索；仅在原生不支持的场景（如 Web 端）使用 Web API。独立 `expo-gaode-map-search` 仅建议历史项目固定到 `2.2.33` 使用。
