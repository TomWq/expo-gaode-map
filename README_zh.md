...
| 一体化能力（地图+搜索+导航+Web API） | ✅ core / navigation 内置搜索能力，Web API 可选 | ❌ 常需多库拼装 | ❌ 主要是地图层能力 |
| 导航能力（路径规划 + 导航视图） | ✅ `expo-gaode-map-navigation` | ❌ | ❌ |
| 搜索能力（POI/周边/地理编码） | ✅ `expo-gaode-map` / `expo-gaode-map-navigation` 内置 + `web-api` | ❌ | ⚠️ 需额外组合 |
...
### 可选模块
- 🔍 **搜索功能** - 已内置在 `expo-gaode-map` 和 `expo-gaode-map-navigation` 中，提供 POI 搜索、周边搜索、关键字搜索、地理编码等
...
- **内置原生搜索**（`expo-gaode-map` / `expo-gaode-map-navigation`）：原生实现，性能更好，需要配置原生环境
